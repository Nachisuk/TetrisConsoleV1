...
namespace TetrisConsoleV1
{
    ...
    public class Tetrimo
    {
        ...
        public bool czyJestCosPonizej()
        {
            for(int i =0; i < 4; i++)
            {
                if (lokacja[i][0] + 1 >= GameBoard.TetrisBoardHeight+1) return true;
                if (lokacja[i][0] + 1 < GameBoard.TetrisBoardHeight+1)
                {
                    if(lokacja[i][0]>=0)
        ...
        public bool czyJestCosZPrawa()
        {
            for (int i = 0; i < 4; i++)
            {
                if (lokacja[i][1] == GameBoard.TetrisBoardWidth-1)
                {
                    return true;
                }
                else if (lokacja[i][0] >= 0 && GameBoard.lokacjaOstatniegoTetrisaGrid[lokacja[i][0], lokacja[i][1] + 1] == 1)
        ...
        {
            for (int i = 0; i < GameBoard.TetrisBoardHeight+1; i++)
            {
            ...
            while (poza == true)
            {
                ...
                for (i = 0; i < 4; i++)
                {
                    if (lokacja[i][1] < 0) //wychodzi za lewą granice
                        Przesun(1, 0);
                    else if (lokacja[i][1] >= GameBoard.TetrisBoardWidth)
                        Przesun(-1,0);
                }
                if (i == 4)
                    poza = false;
